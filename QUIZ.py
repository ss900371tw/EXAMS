            ...
            for index, row in temp_df.iterrows():
                ...
                if student_img is not None:
                    prompt = (
                        ...
                        f"最高配分（滿分）：{row['配分']} 分。\n\n"
                        f"【從寬給分（應變調整）指引】\n"
                        f"1. 觀念正確即給分：只要學生手寫內容、算式或圖表中展現出「核心觀念正確」，即使漏掉部分關鍵字、描述不夠精準或有輕微字詞筆誤，皆應盡量從寬給予滿分。\n"
                        f"2. 同義轉換認可：不盲目進行字面精確對齊。若學生使用的術語、換句話說的表達方式與標準答案意思相近，視為正確。\n"
                        f"3. 局部給分（Partial Credit）從優：若算式推導過程正確但最後計算粗心出錯，或多步驟問題中前半段邏輯正確，不應該視為整題全錯，應酌情扣適當分數。\n"
                        f"4. 錯誤處置：只有在完全答非所問、空白、沒針對問題要求回答或作圖列表或核心觀念嚴重錯誤時，才予以扣除較多分數。\n\n"
                        f"【批改任務說明】\n"
