            ...
            for (cx, cy), (w, h), angle, box_points in candidates:
                ...
                if extracted_text and extracted_text.strip():
                    ...
                    try:
                        ...
                        ocr_prompt = (
                        "你是一個精確的 OCR 文字辨識專家。請將這張圖片中的所有文字提取出來，並遵循以下嚴格規則：\n"
                        "1. 必須依照人類正常的【從上到下、從左到右】閱讀順序輸出，禁止根據文字的空間高低落差任意換行。\n"
                        "2. 針對圖片中的手寫繁體中文，請精確辨識，不要遺漏或看錯任何字，絕對不可以把中文誤判為英文符號。\n"
                        "3. 針對數學公式與計算過程，請直接將其整合成標準的 Markdown 或 LaTeX 格式（例如使用單行 $...$ 或獨立分行 $$...$$），將分子分母整合成水平的一行，禁止用換行來表達分數線。\n"
                        "4. 僅輸出辨識後的文本內容，不要包含任何額外的解釋或空間座標標記。"
                        )
